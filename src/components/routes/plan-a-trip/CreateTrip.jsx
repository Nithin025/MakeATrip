...
function CreateTrip({ createTripPageRef }) {
  ...
  const [formData, setFormData] = useState({}); // ✅ Change from array `[]` to object `{}`
  ...
  const generateTrip = async () => {
    ...
    ) // ✅ Ensures startLocation is sent
      .replace(/{location}/g, location || "Unknown") // ✅ Ensures location is sent
    ...
    try {
      ...
      const trip = await JSON.parse(result.response.text());
      setIsLoading(false);
      SaveTrip(trip);
      console.log(trip);
      ...
      <div className="form mt-14 flex flex-col gap-16 md:gap-20 ">
        ...
        <div className="starting-location">
          ...
          <ReactGoogleAutocomplete
            ...
            onPlaceSelected={(place) => {
              setStartLocation(place.formatted_address); // ✅ Save in state
              handleInputChange("startingLocation", place.formatted_address); // ✅ Save in formData
            }}
        ...
        <div className="place">
          ...
          <ReactGoogleAutocomplete
            ...
            apiKey={import.meta.env.VITE_GOOGLE_MAP_API_KEY}
            onPlaceSelected={(place) => {
              setLocation(place.formatted_address); // ✅ Save in state
              handleInputChange("location", place.formatted_address); // ✅ Save in formData
            }}
